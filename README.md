...
本项目基于 Garfield++ 框架开展气体探测器中电子输运、雪崩倍增及电荷累积效应的仿真研究。Garfield++ 是一个面向气体探测器和半导体探测器的开源仿真工具，主要用于微观输运、电子雪崩、离子漂移以及信号形成等过程的模拟，其源代码由 Garfield++ 开发团队在 CERN GitLab 仓库中维护。

本仓库中的程序主要包括基于 Garfield++ 编写的 THGEM、GMCP 及相关微结构气体探测器的电荷累积仿真代码，以及用于数据统计、结果输出和绘图分析的辅助程序。除特别说明外，本仓库不重新分发 Garfield++ 的原始源代码，用户需要根据 Garfield++ 官方说明自行安装和配置 Garfield++ 环境。

如果本仓库中的部分代码对 Garfield++ 原始程序进行了修改，相应修改仅用于本研究中的多线程雪崩输运计算和电荷累积仿真流程。Garfield++ 原始程序的版权和许可证信息仍归原开发者所有，相关文件中的原始版权声明和许可证说明应予以保留。

在使用本项目代码时，请同时引用 Garfield/Garfield++ 的相关文献或官方资料，并在论文、报告或其他公开成果中说明本项目的仿真程序基于 Garfield++ 框架开发。

## Garfield++ 源码修改说明

本仓库中包含 Garfield++ 原始源码的局部修改版本，所有修改均基于 Garfield++ 原始开源代码完成。修改内容主要包括事件级并行计算流程、线程局部对象管理、随机数处理以及结果合并输出等部分。相关修改不代表 Garfield++ 官方版本，仅用于本研究中的特定仿真任务。

本仓库保留 Garfield++ 原始源码文件中的版权声明和许可证信息。用户在使用、修改或再发布相关代码时，应同时遵守 Garfield++ 原始许可证以及本仓库中的说明。
