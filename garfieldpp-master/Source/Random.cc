...
static Garfield::RandomEngineRoot g_seedSource;
...
unsigned int RootSeed(const uint64_t seed) {
  const uint64_t mixed = SplitMix64(seed);
  unsigned int s = static_cast<unsigned int>(
      (mixed & 0xffffffffULL) ^ (mixed >> 32));
  if (s == 0u) s = 1u;
  return s;
}

unsigned int NextThreadSeed() {
  std::lock_guard<std::mutex> lock(g_seedMutex);
  unsigned int seed =
      1u + static_cast<unsigned int>(g_seedSource.Draw() * 4294967294.0);
  if (seed == 0u) seed = 1u;
  return seed;
}

std::atomic<uint64_t> g_epoch{1};
thread_local Garfield::RandomEngineRoot g_threadEngine;
thread_local bool g_threadEngineInitialised = false;
thread_local uint64_t g_threadSeedTag = 0u;
thread_local unsigned int g_threadEngineSeed = 0u;

Garfield::RandomEngineRoot& ThreadEngine() {
  if (!g_threadEngineInitialised) {
    g_threadEngineSeed = NextThreadSeed();
    g_threadSeedTag = g_threadEngineSeed;
    g_threadEngine.SetSeed(g_threadEngineSeed);
    g_threadEngineInitialised = true;
  }
  return g_threadEngine;
}
...
double Garfield::Random::Draw() noexcept {
  return draw();
}

void Garfield::Random::SetThreadSeed(const uint64_t seed) {
  ::g_threadSeedTag = seed;
  ::g_threadEngineSeed = ::RootSeed(seed);
  ::g_threadEngine.SetSeed(::g_threadEngineSeed);
  ::g_threadEngineInitialised = true;
  ::g_epoch.fetch_add(1, std::memory_order_acq_rel);
}

uint64_t Garfield::Random::GetThreadSeed() noexcept {
  return ::g_threadSeedTag;
}

uint64_t Garfield::Random::GetThreadEngineSeed() noexcept {
  return static_cast<uint64_t>(::g_threadEngineSeed);
}

uint64_t Garfield::Random::Epoch() noexcept {
  return ::g_epoch.load(std::memory_order_acquire);
}

void Garfield::Random::InvalidateBuffers() noexcept {
  ::g_epoch.fetch_add(1, std::memory_order_acq_rel);
}

std::function<double(void)> Garfield::Random::draw{
    []() -> double {
      return ThreadEngine().Draw();
    }};
