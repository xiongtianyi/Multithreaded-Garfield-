...
#endif
{
  ...
  if (type == ElectronCollisionTypeIonisation &&
      m_avalancheProbabilityScale < 1. &&
      RndmUniform() > m_avalancheProbabilityScale) {
    // Keep the selected energy-loss process but suppress the creation
    // of ionisation secondaries for empirical gain calibration.
    type = ElectronCollisionTypeInelastic;
  }
  #ifndef __GPUCOMPILE__
  // Increase the collision counters.
  if (m_countCollisions) {
    ++m_nCollisions[type];
  ...
  if (type == ElectronCollisionTypeIonisation) {
    ...
    if (m_useDeexcitation && m_iDeexcitation[level] >= 0) {
      ...
      if (loss * m_rgas[igas] > m_minIonPot &&
          RndmUniform() < m_rPenning[level] * m_avalancheProbabilityScale) {
        ...
        if (m_countCollisions) ++m_nPenning;
      }
    }
  }
  #endif
...
bool MediumMagboltz::PhotonCollision(const double e, int& type, int& level,
  ...
  if (m_countCollisions) ++m_nPhotonCollisions[type];
  // Ionising collision
  if (type == 1) {
    Secondary electron;
