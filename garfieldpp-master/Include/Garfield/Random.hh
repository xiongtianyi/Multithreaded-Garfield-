...
class Random
{
  public:
    ...
    static void SetThreadSeed(const uint64_t seed);
    static uint64_t GetThreadSeed() noexcept;
    static uint64_t GetThreadEngineSeed() noexcept;
    static uint64_t Epoch() noexcept;
    static void InvalidateBuffers() noexcept;
  private:
...
inline double RndmGaussian() {
  ...
  thread_local double u = 0.;
  thread_local uint64_t cacheEpoch = uint64_t(-1);
  const uint64_t epoch = Random::Epoch();
  if (cacheEpoch != epoch) {
    cached = false;
    cacheEpoch = epoch;
  }
